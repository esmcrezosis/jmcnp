...
import com.esmc.mcnp.web.mvc.model.echange.BcEchange;
import com.esmc.mcnp.web.mvc.utils.BaseRestController;

@RestController
@RequestMapping(value = "/appro")
public class ApproController extends BaseRestController {
	...
	public ApproController(EuCompteService compteService, EuCompteCreditService compteCreditService,
	...
	public ResponseEntity<?> approBc(BcEchange bcEchange) {
		if (bcEchange.getBc() > 0 && StringUtils.isNotBlank(bcEchange.getCodeMembre())
			...
			if ((Objects.nonNull(membre) && membre.getDesactiver() == 0)
				...
				try {
					if (!credits.isEmpty()) {
						...
						if (soldeCredit >= bcEchange.getBc()) {
							List<Long> ids = credits.stream().map(EuCompteCredit::getIdCredit)
							...
							EuCompte compteDest = compteService.createOrUpdate("NB", "TBC",
									bcEchange.getCodeMembreDest(), bcEchange.getBc());
							...
							return ResponseEntity.badRequest()
									.body(new Result(1, "Echec de l'opération: Solde insuffisant"));
						}
					} else {
						return ResponseEntity.badRequest()
								.body(new Result(1, "Echec de l'opération: Aucune ligne de BC correspondante trouvée"));
					}
				} catch (Exception e) {
				...
				return ResponseEntity.badRequest()
						.body(new Result(1, "Echec de l'opération: Votre compte n'est pas activé"));
			}
		}
		return ResponseEntity.badRequest().body(new Result(1, "Echec de l'opération: Donées insuffisantes"));
	}
}
